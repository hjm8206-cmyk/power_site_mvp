...
function bindEvents() {
  ...
  document.getElementById("clearTowerButton").addEventListener("click", clearTowers);
  document.getElementById("clearRoadButton").addEventListener("click", () => clearManualRoad());
  document.getElementById("saveManualRoadButton")?.addEventListener("click", saveManualRoad);
  document.getElementById("recalculateRoadButton")?.addEventListener("click", recalculateManualRoad);
...
function initMap() {
  ...
  if (!window.kakao || !kakao.maps) {
    ...
    fallback.textContent = "Kakao 지도 JS 로딩 실패. KAKAO_JS_KEY와 JavaScript 플랫폼 도메인(http://localhost:8501)을 확인하세요.";
    return;
  }
...
function renderInitialNotices() {
  ...
  if (!status.kakao_js) notices.push("KAKAO_JS_KEY가 없어 지도가 표시되지 않습니다. Kakao JavaScript 플랫폼 도메인을 http://localhost:8501 로 등록하세요.");
  if (!status.vworld) notices.push("VWORLD_API_KEY가 없어 필지, 용도지역, 도로, 건물 자동조회는 수동확인으로 표시합니다.");
  notices.push(`VWorld 서비스 URL 기준: ${window.APP_CONFIG?.vworldDomain || "http://localhost:8501"}`);
...
function drawParcels(parcelGroup) {
  ...
  (parcelGroup?.display_adjacent || parcelGroup?.adjacent || []).forEach((parcel) => {
...
function toggleRoadMode(force) {
  ...
  button.textContent = state.roadMode ? "도로 찍는 중" : "위성도로 마킹";
}
...
function addManualRoadPoint(latLng) {
  ...
  const marker = new kakao.maps.CustomOverlay({
    position: latLng,
    content: `<button type="button" class="road-marker" title="위성도로 후보 ${index}">${index}</button>`,
    xAnchor: 0.5,
    yAnchor: 0.5,
    map: state.map
  });
...
function saveManualRoad() {
  attachManualRoadToAnalysis();
  refreshManualRoadLine();
  refreshScore();
  const count = getManualRoadPoints().length;
  renderNotices(count >= 2 ? ["수동 도로로 저장했습니다. 선택 부지군과 접도 여부를 재계산합니다."] : ["도로 위를 2점 이상 클릭한 뒤 저장하세요."]);
}

function recalculateManualRoad() {
  attachManualRoadToAnalysis();
  refreshManualRoadLine();
  refreshScore();
}
...
function refreshManualRoadLine() {
  ...
  document.getElementById("manualRoadSummary").textContent = `위성도로 후보 ${points.length}점`;
...
function refreshManualRoadLine() {
  ...
  if (summary) summary.textContent = `수동도로 후보 ${points.length}점 / ${width}`;
  if (!state.map || points.length < 2) return;
...
function removeManualParcel() {
  ...
  const targetIds = selectedManualIds.length ? selectedManualIds : manualIds;
  if (!targetIds.length) {
    renderNotices(["삭제할 수동 추가 필지가 없습니다. 수동 추가 필지를 선택하거나 마지막 추가 필지를 삭제하세요."]);
    return;
  }
  group.manual_added_ids = (group.manual_added_ids || []).filter((id) => !targetIds.includes(String(id)));
  group.adjacent = (group.adjacent || []).filter((item) => !targetIds.includes(String(item.id)));
  group.display_adjacent = (group.display_adjacent || []).filter((item) => !targetIds.includes(String(item.id)));
  group.displayed_parcels = (group.displayed_parcels || []).filter((item) => !targetIds.includes(String(item.id)));
  group.nearby_parcels = (group.nearby_parcels || []).filter(
    (item) => !targetIds.includes(String(item.id)) || !(item.role === "manual_added" || item.manual_added)
  );
  targetIds.forEach((id) => {
    state.selectedParcelIds.delete(id);
    const polygon = state.adjacentParcelPolygons.get(id);
    if (polygon) polygon.setMap(null);
    state.adjacentParcelPolygons.delete(id);
  });
  renderNotices([`수동 추가 필지 ${targetIds.length}개를 제거했습니다.`]);
  updateParcelStyles();
  refreshScore();
}
...
function collectManual() {
  const slopeBand = document.getElementById("manualSlopeBand")?.value || "auto";
...
function resetManualControlsForNewAddress() {
  ...
  if (manualSlopeBand) manualSlopeBand.value = "auto";
}
...
function resetInteractiveState() {
  ...
  document.getElementById("manualRoadSummary").textContent = "위성도로 후보 0점";
}
...
function updateResultOverview(data, score) {
  ...
  const decision = gradeDecisionLabel(grade);
  const areaBlocked = Boolean(score.evaluation_blocked);
...
function renderFacts(data) {
  ...
  const facts = [
    ...
    ["농림지역 감점", scoreMetrics.agricultural_penalty ? `-${scoreMetrics.agricultural_penalty}` : "미적용"],
    ...
    ["위성도로 수동판독", scoreMetrics.manual_visual_road?.ok ? `${scoreMetrics.manual_visual_road.distance_m ?? "-"} m / ${scoreMetrics.manual_visual_road.width_class || "폭원 미확인"}` : "미사용"],
    ...
    ["민가밀집 기본점수", densityScore !== undefined ? `${densityScore} / 10` : "-"],
    ...
    ["송전축 전압", voltageLabel],
    ["전력축 위치점수", powerDistanceScore !== undefined ? `${powerDistanceScore} / 20` : "-"],
    ["전력축 전압점수", powerVoltageScore !== undefined ? `${powerVoltageScore} / 10` : "-"],
    ["전력축 인접성 점수", powerAxisScore !== undefined ? `${powerAxisScore} / 30` : "-"],
    ["선하지·안전거리·한전협의 필요", transmission.power_axis_needs_safety_review ? "필요" : "일반 확인"],
    ["도로·접도·공사차량 진입 점수", roadScore !== undefined ? `${roadScore} / 20` : "-"],
    ...
    ["기본점수 합계", data.score?.base_total !== undefined ? `${data.score.base_total} / 100` : "-"],
    ...
    ["정책입지 점수", policyScore !== undefined ? `${policyScore} / 10` : "-"],
    ...
    ["전력자립도 내부점수", powerSelfScore !== undefined ? `${powerSelfScore} / 5` : (policy.power_self_internal_score !== undefined && policy.power_self_internal_score !== null ? `${policy.power_self_internal_score} / 5` : "-")],
...
function renderPolicyReferenceBlock(data) {
  ...
  const missing = !policy.ok || matchStatus.includes("기준자료 없음") || matchStatus.includes("업데이트 필요");
