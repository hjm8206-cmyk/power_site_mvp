...
def classify_sensitive_facility(item_or_text: Any) -> str:
    ...
    if any(keyword in text for keyword in ["중학교", "고등학교", "학교"]):
        return "학교"
    if any(keyword in text for keyword in ["요양병원", "요양원", "노인복지"]):
        return "요양·노인복지시설"
    if "병원" in text:
        return "병원"
...
def calculate_sensitive_facility_penalty(
    ...
    reference_items = [item for item in items if (_num(item.get("applied_distance_m")) or 999999) <= 1000]
    ...
    if distance is not None and distance <= 250:
        penalty = 30
        fatal_cap = 55
        judgement = "민감시설이 부지경계 기준 250m 이내에 있어 주민수용성 리스크로 강하게 감점했습니다."
    elif distance is not None and distance <= 500:
        penalty = 15
        fatal_cap = 75
        judgement = "민감시설이 부지경계 기준 500m 이내에 있어 주민수용성 리스크로 감점했습니다."
    elif distance is not None and distance <= 1000:
        judgement = "민감시설이 500m 초과 1km 이내에 있어 참고값으로 표시합니다. 자동감점은 적용하지 않았습니다."

    return {
        "detection_status": "자동탐지 완료" if reference_items else "자동탐지 결과 없음",
        "penalty": penalty,
        "fatal_cap": fatal_cap,
        ...
        "distance_from_anchor_m": (nearest or {}).get("distance_from_anchor_m"),
        "distance_from_site_boundary_m": (nearest or {}).get("distance_from_site_boundary_m"),
        "applied_distance_m": _round(distance),
        "source": _source(nearest),
        "confidence": "높음" if nearest else "낮음",
        "judgement": judgement,
    }
