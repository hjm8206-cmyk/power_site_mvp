...
def derive_metrics(
    ...
    zoning_text = _zoning_text(zoning, permit, selected_zoning_values)
...
def calculate_permit_score(metrics: Dict[str, Any]) -> Tuple[float, str]:
    ...
    metrics["integrated_zoning_reason"] = lowest.get("reason")
    ...
    if lowest.get("scope") != "기준 필지":
        return float(lowest.get("score") or 0), f"추가필지 {lowest.get('parcel_id') or ''}의 {lowest.get('zoning') or '미확인'} 용도지역이 가장 낮은 인허가 점수로 확인되어 통합 용도지역 점수에 적용했습니다."
    return float(lowest.get("score") or 0), str(lowest.get("reason") or "용도지역 기준 점수를 반영했습니다.")
...
def calculate_fatal_cap(metrics: Dict[str, Any]) -> Tuple[Optional[float], List[str]]:
    ...
    cap_map = [
        ...
        ("fragmentation_fatal_cap", "필지 쪼개짐 상한"),
    ]
...
def _penalty_items(metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    add(
        ...
        metrics.get("fragmentation_judgement") or "개발부지 필지 수가 많아 권리확보와 집적성 리스크를 반영했습니다.",
    )

    zoning_text = metrics.get("zoning_text") or ""
    if metrics.get("is_greenbelt"):
        metrics["greenbelt_penalty"] = 25
        metrics["greenbelt_fatal_cap"] = None
        add(
            "greenbelt_penalty",
            "개발제한구역 감점",
            25,
            "개발제한구역으로 확인되어 인허가 리스크를 별도 감점으로 반영했습니다. 단, 공공성, 정책사업, 도시관리계획 변경, 지자체 추진 의지가 명확한 경우 특수검토 후보로 남길 수 있습니다.",
        )
    else:
        metrics["greenbelt_penalty"] = 0

    if metrics.get("is_agricultural"):
...
def _fragmentation_profile(development_parcel_count: Any) -> Dict[str, Any]:
    ...
    if count <= 7:
        return {"penalty": 0, "fatal_cap": None, "judgement": "개발부지 필지 수가 7개 이하로 필지 쪼개짐 감점은 적용하지 않습니다."}
    if count <= 12:
        return {"penalty": 3, "fatal_cap": None, "judgement": "개발부지 필지 수가 8~12개로 권리확보와 집적성 보완이 필요합니다."}
    if count <= 20:
        return {"penalty": 8, "fatal_cap": None, "judgement": "개발부지 필지 수가 13~20개로 필지 쪼개짐 리스크가 큽니다."}
    if count >= 30:
        return {"penalty": 15, "fatal_cap": 70, "judgement": "개발부지 필지 수가 30개 이상으로 필지 쪼개짐 리스크가 매우 큽니다."}
    return {"penalty": 15, "fatal_cap": 80, "judgement": "개발부지 필지 수가 21개 이상으로 비연속·권리확보 리스크를 확인해야 합니다."}
...
def _is_sensitive_facility_use(building_use: str) -> bool:
    return any(
        ...
        for keyword in ["학교", "병원", "요양", "어린이집", "유치원", "보육", "의료", "어린이공원", "노인복지"]
    )
